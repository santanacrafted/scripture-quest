...
@Component({
  ...
  styles:[`
    .avatar-trigger{position:fixed;z-index:40;top:calc(env(safe-area-inset-top) + 1rem);right:1rem;display:grid;width:46px;height:46px;place-items:center;border:2px solid #f5d36e;border-radius:50%;background:linear-gradient(#326f62,#193e43);color:#fff8df;font:900 1.25rem Georgia;box-shadow:inset 0 0 0 2px #0e1715,0 7px 18px #0007}
    .menu-backdrop{position:fixed;z-index:80;inset:0;background:#020908a8;backdrop-filter:blur(3px)}
    .side-menu{position:fixed;z-index:81;top:0;right:0;display:grid;grid-template-rows:auto 1fr auto;width:min(88vw,360px);height:100svh;padding:calc(env(safe-area-inset-top) + 1rem) 1rem calc(env(safe-area-inset-bottom) + 1rem);background:linear-gradient(160deg,#123b34,#061411 75%);color:#fff8df;box-shadow:-18px 0 45px #0009;transform:translateX(105%);transition:transform 240ms cubic-bezier(.2,.8,.2,1);pointer-events:none}.side-menu.open{transform:none;pointer-events:auto}
