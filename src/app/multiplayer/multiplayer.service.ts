...
export class MultiplayerService {
 ...
 spinWheel(matchId:string):Match { const m=this.must(matchId); this.assertTurn(m); const category=MULTIPLAYER_CATEGORIES[Math.floor(Math.random()*5)]; return this.update({...m,selectedCategory:category,phase:'question',lastTurnSummary:`The wheel landed on ${category}.`}); }
 simulateOpponent(matchId:string,playerId:string):Match { let m=this.must(matchId);if(m.currentPlayerId===playerId||m.status!=='active')return m;if(m.phase==='spin')m=this.spinWheel(matchId);const q=this.questions.getRandomQuestionByCategory(m.selectedCategory!,m.phase!=='question');const choice=Math.random()<.48?q.correctAnswer:q.choices.find(x=>x!==q.correctAnswer)!;return this.submitAnswer(matchId,m.currentPlayerId!,q.id,choice).match; }
 ...
 private assertTurn(m:Match){if(!m.currentPlayerId||m.status!=='active')throw Error('This match is not active.');}
